...
- **Conteúdo permitido:** Provedores de estado global (`contexts/`), hooks utilitários (`hooks/`), lógicas de áudio/frequência (`utils/`), tipagens globais e componentes globais e reutilizáveis de controle (como guards de orientação).
...
- **Diretiva de Banco de Dados Local:** O agente deve ler e criar bancos de dados do simulado exclusivamente como arquivos JSON dentro do subdiretório `src/modules/curso-msa/questions/`.
...
1.  **Timbre:** Utilizar obrigatoriamente osciladores com onda **triangle** combinada com filtros passa-baixa (lowpass filter em `1000Hz`) para criar um som de órgão eletrônico quente e suave. **Evite ondas senoidais agressivas.**
